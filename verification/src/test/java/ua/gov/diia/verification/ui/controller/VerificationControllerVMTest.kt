...
class VerificationControllerVMTest(
    ...
    private lateinit var errorHandlingBehaviour: WithErrorHandling
    private lateinit var verificationMethods: List<VerificationMethod>
    private lateinit var viewModel: TestVerificationControllerVM

    @Before
    fun setUp() {
        MockitoAnnotations.openMocks(this)
        errorHandlingBehaviour = StubErrorHandler()
        verificationMethods = listOf(
            StubVerificationMethod(false, "test_nav", true),
            StubVerificationMethod(true, "test_url", true),
            StubVerificationMethod(false, "test_disabled", false)
        )
        viewModel = TestVerificationControllerVM(
    ...
    fun `external verification`() = runTest {
        ...
        verify(applicationLauncher, atLeast(1)).launch("http://test.com/1329")
    ...
    fun `complete verification`() = runTest {
        ...
        viewModel.startVerification(schema)
        viewModel.awaitUserVerifying()
        viewModel.handleVerificationResult(VerificationFlowResult.VerificationMethod(method.name))
        viewModel.awaitUserVerifying()
