...
@Composable
fun TableBlockTwoColumnsPlainOrgLandscape(
    modifier: Modifier = Modifier,
    data: TableBlockTwoColumnsPlainOrgData,
    onUIAction: (UIAction) -> Unit
) {
    val currentPhoto = remember { mutableStateOf(data.photo) }
            ...
            if (!data.items.isNullOrEmpty()) {
                val items = buildList {
                    ...
                    if (data.items.size > 2) {
                        val reminder = data.items.size % 3
                        if (reminder != 0) {
                            repeat(3 - reminder) {
                                add(TableItemVerticalMlcData())
                            }
                        }
                    }
                }
                items.chunked(3).forEach { rowList ->
                    Column(
...
@Composable
fun TableBlockRow(
    rowItems: List<TableBlockItem>? = null,
    onUIAction: (UIAction) -> Unit
) {
    Row(
        modifier = Modifier
...
fun TableBlockTwoColumnsPlainOrgLandscapeDataPreview() {
    val heading = HeadingWithSubtitlesMlcData(
        value = "Дія Надія Володимирівна",
        subtitles = listOf("Diia Nadiia")
    )
    ...
    val data = TableBlockTwoColumnsPlainOrgData(
        id = "123",
        heading = heading,
        photo = base64String,
        items = listOf(
...
@Composable
@Preview(heightDp = 360, widthDp = 800, showBackground = true)
fun TableBlockTwoColumnsPlainOrgLandscapeDataPreview_NoPhoto() {
    val heading = HeadingWithSubtitlesMlcData(
        value = "Дія Надія Володимирівна",
        subtitles = listOf("Diia Nadiia")
    )
    ...
    val data = TableBlockTwoColumnsPlainOrgData(
        ...
        items = listOf(
            ...
            TableItemVerticalMlcData(
                id = "04",
                value = UiText.DynamicString("Загальноосвітня школа I-III ступенів №4 мю Білгорода-Дністровського Одеської Області")
            )
        )
    )
...
@Composable
@Preview(heightDp = 360, widthDp = 800, showBackground = true)
fun TableBlockTwoColumnsPlainOrgLandscapeDataPreview_NoPhoto_Two() {
    val heading = HeadingWithSubtitlesMlcData(
        value = "Дія Надія Володимирівна",
        subtitles = listOf("Diia Nadiia")
    )
