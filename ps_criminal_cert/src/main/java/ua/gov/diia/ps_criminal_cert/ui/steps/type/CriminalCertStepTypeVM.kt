...
import ua.gov.diia.ps_criminal_cert.ui.CriminalCertConst.SCREEN_TYPE_ACTION_CREATE_APPLICATION
...
class CriminalCertStepTypeVM @Inject constructor(
    ...
    fun doInit(navBarTitle: String?, reasons: String) {
    ...
    fun onUIAction(event: UIAction) {
        ...
        when (action?.type) {
            SCREEN_TYPE_ACTION_CREATE_APPLICATION -> {
                createApplication()
            }
        }
    }
    ...
    fun getScreenContent() {
        ...
        {
            api.getCriminalCertTypes().let { response ->
    ...
    private fun createApplication() {
        executeActionOnFlow(
            ...
            val request = CriminalCertApplicationRequest(
                type = selectedTypeId,
                reasonCode = reason
            )
            api.createCriminalCertApplication(request).let { response ->
                if (response.template != null) {
